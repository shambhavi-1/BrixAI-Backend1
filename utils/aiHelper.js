...
// Classify issue severity using Grok API
exports.classifyIssueSeverity = async (description, imageUrl = null) => {
  try {
    let prompt = `Classify the severity of this construction issue: "${description}".
    Options: critical, high, medium, low. Consider safety risks, project impact, and urgency. Respond with only the severity level.`;

    if (imageUrl) {
      prompt += ` Image description: Analyze the image for construction issues and classify severity accordingly.`;
    }
    ...
    const severity = response.data.choices[0].message.content.trim().toLowerCase();
    return ['critical', 'high', 'medium', 'low'].includes(severity) ? severity : 'medium';
  } catch (error) {
    console.error('Error classifying severity with Grok:', error);
...
// Analyze image for issues using Groq
exports.analyzeImageForIssues = async (imageUrl, description = '') => {
  try {
    const prompt = `Analyze this construction site image for potential issues, risks, or problems.
    Description: "${description}"
    Image URL: ${imageUrl}

    Identify any safety hazards, structural issues, material problems, or other concerns.
    Classify severity and suggest actions. Respond in JSON format: {issues: [array of issues], severity: "level", recommendations: [array]}`;
    ...
    return { issues: [], severity: 'low', recommendations: [] };
  }
};

// Convert voice to text using Grok
exports.transcribeAudio = async (audioUrl) => {
  try {
    const prompt = `Transcribe this audio file to text. Audio URL: ${audioUrl}. Provide only the transcription.`;
    ...
    console.error('Error transcribing audio with Grok:', error);
    return '';
  }
};
